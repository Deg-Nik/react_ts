...
Требования:
Создайте компонент Homework_13.tsx в папке Homework13. По примеру как мы делали на уроке: В папке Homework_13 создайте папку components, а в ней 3 компонента: BlogManagement, Card, Message

В компоненте BlogManagement.tsx дожны быть:
textarea(отдельным компонентом делать не нужно) - в которую вы будете вводить сообщение, которое хотите запостить в своем блоге
...
Компонент Message.tsx должен отбражать сообщение из стейта, которое вы кладете при клике на кнопку "Запостить"
